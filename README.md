...
| 🔭 **多 session 流** | 每个 Claude Code 会话独立 URL，JSONL 持久化历史。可在浏览器里 ➕ 新建 / × 删除。 |
...
| 🔭 **Per-session feed** | Each Claude Code session gets its own URL and persistent JSONL history. Add ➕ / delete × from the sidebar. |
...
> it, Claude says "资料未覆盖" (not covered) and stops.
